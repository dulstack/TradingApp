...
std::string DB::list_curr(){
 ...
 if(sqlite3_prepare(db,"SELECT name, price FROM currencies ORDER BY ID ASC;",-1,&query, NULL)){
  ...
  //output the query in the result string(res)
  while(sqlite3_step(query)!=SQLITE_DONE){
   std::string curr_name=(const char*)sqlite3_column_text(query, 0), curr_price=(const char*)sqlite3_column_text(query, 1);
   res=res+std::string("Name: ")+curr_name+std::string("\nPrice: ")+curr_price+"\n\n";
  }
 }
